...
enum _Environment {
  ...
  const _Environment(this.env, {required this.queryParams});

  factory _Environment.forEnv(String envString) {
    final env = envString.toLowerCase();
    return _Environment.values.firstWhere((it) => it.env == env, orElse: () => _Environment.simpsons);
  }
}
...
class Config {
  ...
  static const isPublicBuild = bool.fromEnvironment('PUBLIC_BUILD', defaultValue: true);

  static final _env = _Environment.forEnv(const String.fromEnvironment('ENV'));

  Uri get aiApiUrl => Uri.parse('https://openrouter.ai/api/v1');
